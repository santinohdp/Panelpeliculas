...
function obtenerPeliculas(req, username, password) {
  ...
  const streams = filas.map((c, i) => {
    ...
    const urlReproduccion = `${base}/movie/${encodeURIComponent(username)}/${encodeURIComponent(password)}/${c.id}.${ext}`;
    return {
      num: i + 1,
      name: c.titulo,
      stream_type: 'movie',
      ...
      custom_sid: '',
      direct_source: urlReproduccion,
      tmdb_id: c.tmdb_id || '',
      // Campos extra para apps que no siguen el esquema estándar de Xtream
      // y esperan directamente "url" / "img" / "id" / "category" (ej: Magma).
      id: c.id,
      url: urlReproduccion,
      img: c.logo || '',
      category: cat ? Number(cat.category_id) : 0,
    };
  });
...
function obtenerInfoSerie(req, username, password, seriesId) {
  ...
  for (const ep of encontrado.episodios) {
    ...
    const urlReproduccion = `${base}/series/${encodeURIComponent(username)}/${encodeURIComponent(password)}/${ep.id}.${ext}`;
...
module.exports.manejarSeriesStream = manejarSeriesStream;  const activo = credencialesValidas && !!u.activo;
  const vencido = credencialesValidas && u.fecha_vencimiento < hoyISO();
  const auth = credencialesValidas && activo && !vencido;
